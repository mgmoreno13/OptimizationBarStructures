...
for (int i = 0; i < Grafobarras.size(); ++i)
{
	printf(" %d -",i);
	for (int j = 0;j < Grafobarras[i].size(); ++j)
	{
		printf(" %d ", Grafobarras[i][j]);
	}
	printf("\n");
}*/
...
char buf[150];
sprintf(buf, "%d_%d_meme_bybarras.txt",nelem, prueba);
...
//https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/ - modified
void BFS_rand(int n){

//printf("NODO DE INICIO %d \n",n);
   ...
   // list<int>::iterator i; 
    ...
    { 
    	...
    	if (k>1 && aux.size()>0){
    		    	//printf("papa %d nodo %d\n",n_aux.second,n_aux.first);
    		    	//printf("hijos : ");
    	for (int i = 0; i < aux.size(); ++i)
        {	
        	
        	if (n_aux.second!=aux[i])//si no es el padre colocar en false para que sea elegible
        	{
        		//printf(" %d ",aux[i]);
        		visitados[aux[i]]=false;
        	}
       ...
       // printf("\n");
        ...
        // vertex n but random. If a adjacent has not been visited,  
        // then mark it visited and enqueue it 
        for (int i = 0; i<aux.size(); ++i) 
        { 
            if (!visitados[aux[i]]) 
            { 
            	//printf("nodo escogidoooo %d\n",aux[i] );
                visitados[aux[i]] = true; 
                queue.push_back(make_pair(aux[i],n1.first)); 
                //printf("nodo %d - %d\n",n,aux[i]);
                bars.push_back(MatrixAdy[n1.first][aux[i]]);
                cont_bfs++;
                if (cont_bfs>=num1)
                {
                	bandera=true;
	...
	Individuo Indi;
		...
		for (int k = 0; k < tam_pob;k=k+2)//OJO
		{
			...
			if(num<0.8){//PROBABILIDAD DE CROSSOVER
                ...
                n=rand()%(Grafo.size());//en que barra empieza
				num1=rand()%(Grafo.size());//maximo numero de barras 
