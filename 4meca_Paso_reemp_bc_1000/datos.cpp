          ...
          double** &vecto1, double** &deslm, double** &tenlm, double** &vecth)
{
	...
	int ndato, nbar, nCatalogos, auxa, nCatArm, nCatAcRF, nCatAcRC, nCatConcr;
	int iArm, iRF, iRC, iCon;
    ...
    if(ncaso ==2)fscanf(fp5, "%s %d", letrero, &nvpro);
	...
	fprintf(fp16,"%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d \n",
			npnod, nelem, npres, ncarg, ntipo, ngdln, nmats, iwrit, isale, ndime,
			nreso, nincl, nfami, ncaso, nvpro);

	if(PideMemoria(npnod,nelem,npres,ncarg,ngdln,nmats,ndime,nreso,nincl,
				   mnode,nprop,nevab,indso,threads,lnods,inpre,nareas,narerf,narerc,
                   ...
                   efi_max,ncaso,nvpro,tenlo,mvalor,aux,aux1,mvector,vecto1,deslm,
                   tenlm,vecth)) return -2;
    
    // printf(" pidio memoria \n");
